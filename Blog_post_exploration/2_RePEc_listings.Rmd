...
NOTE: attributing citations to year of publication and not year an reader cites the previous work (which I will consider later in network effects).
```{r, results='asis'}
...
Author_cumulative_citations.data <- 
  Author_citation_count.data %>%
  arrange(author_name, publication_date) %>%
  group_by(author_name) %>%
  mutate(cum_citations = cumsum(total_citations),
         since_first = c(1:n())-1)
...
Author_cumulative_citations.data %>%
  ...
  geom_point()
## MAKE ZERO ENTRIES FOR WHEN NO ENTRIES UP TO 40
