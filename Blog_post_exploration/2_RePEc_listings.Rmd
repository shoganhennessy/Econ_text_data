...
Journals.data %>% head(10) %>% 
  select(-c('abstract_views', 'downloads', 'publisher', 'items_listed')) %>% 
  knitr::kable(format = 'markdown')
```
...
Top5_since.data <- Author_cumulative_citations.data %>%
  ...
  group_by(since_first_top5) %>%
  summarise(mean_citations = mean(total_citations),
            count = n()) %>%
  mutate(cum_citations = cumsum(mean_citations)) 

Top5_since.data %>%
  ggplot(aes(x=since_first_top5, y = cum_citations)) + 
  geom_point()
