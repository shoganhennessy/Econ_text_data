...
Economists_citations.data %>%
  ...
  geom_histogram(aes(y = ..count..), colour='black', fill='#FF6666', alpha=0.2) +
...
Journals_articles.data %>% head(10) %>% knitr::kable(format = 'markdown')
```
...
Author_cumulative_citations.data %>%
  ...
  scale_y_continuous(name = 'Mean Citations', limits = c(0,80))
```
...
Top5.eventstudy <- Top5_since.data %>%
  ...
  ylim(0, 1) + geom_smooth(span = 1, se = F) +
  geom_smooth(aes(x = coefficient, y = before), method = 'lm', se = F, colour = 'red') +
  geom_smooth(aes(x = coefficient, y = after ), method = 'lm', se = F, colour = 'red')
