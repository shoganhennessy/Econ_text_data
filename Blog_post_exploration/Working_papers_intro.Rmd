...
let's show test this <i>empirically</i>.

Sentiment analysis is commonly applied to text data as an easy coding operation
to measure sentiment -- commonly to measure happiness, or perhaps [1am angry tweets](http://varianceexplained.org/r/trump-tweets/) -- and the same process can textually measure rate of empiricism.
First I'll form a list of a hundred words most likely to appear in abstracts that use the words
'empirical', 'instrument', 'metrics' -- denoting these as empirical papers.
Also list the top hundred words in papers that use the words 
'theretical', 'theory', and 'general equilibrium' -- denoting these theoretical papers.  
This words listing can play the role of a sentiment dictionary (call it a a pseudo sentiment lexicon) with a 
...
empirical_theoretical_words <- Papers_abtracts.data %>%
  ...
  mutate(theoretical = 0 < (
    stri_detect_fixed(abstract, 'theor') + 
    stri_detect_fixed(abstract, 'equilibrium'))) %>%
...
empirical_words <- empirical_theoretical_words %>%
  ...
  arrange(logratio) %>% group_by(logratio > 0) %>% top_n(100, logratio) %>%
  ungroup() %>% mutate(word = reorder(word, logratio))
...
Here are the top 10 for empirical papers with scores : `r top_empirical_words`.

And for theoretical papers: `r top_theoretical_words`.
...
Take a step back and make sure the words make sense -- one would hope that 'econometrics' is an empirical word, and that 
'theory' is a theoretical word!
