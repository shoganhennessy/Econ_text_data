...
Yet the publication rate has not been constant.  What explains the changes in publication rate among these working papers?
...
relative_abstract_words <- abstracts_words %>%
  ...
  filter(published + unpublished > 50) %>% ungroup() %>%
...
```
* Coal is least likely to be in a published paper, with many papers tackling the economic issues of coal production and
pollution (see WP 23417, 26551, etc.) yet apparently going on to make the lowest rate of successful publication.
Comment on ``cocaine'' 

* Comment on unpublished words
* ``afdc'' (WP 7240)
...
Form a pseudo dictionary of words related to empiricism, and then run the ``sentiment'' analysis with this lexicon.
Test the empirism of published articles over time.

```{r, echo=T}
# Which words coincide with data or empirical?
empirical_words <- Papers_abtracts.data %>%
  mutate(empirical = stri_detect_fixed(abstract, 'empiric') > 0) %>%
  ...
  arrange(logratio) %>% group_by(logratio > 0) %>% top_n(100, abs(logratio)) %>%
  ungroup() %>% mutate(word = reorder(word, logratio)) %>% pull(word)
