...
In the early days, around 60\% of the working papers are published in peer reviewed journals, going on to peak at 94\% in 1987 before falling to common day.  What drives this ride and fall?
It is clear that papers since 2015 are still in the pipeline, afterall the NBER series was started
partially to disseminate knowledge current knowledge in light of publishing lags.
...
[post](http://varianceexplained.org/r/trump-tweets/#fnref:fullcode) to investigate the text
...
published_words <- abstracts_words %>% filter(published == T) %>% head(10) %>%
  pull(word) %>% paste(sep = ', ', collapse = ', ') 
unpublished_words <- abstracts_words %>% filter(published == F) %>% head(10) %>%
  pull(word) %>% paste(sep = ', ', collapse = ', ') 
```
...
relative_abstract_words <- abstracts_words %>%
  filter(count >= 5) %>%
...
```
* Bubble is th word most likely to be among published papers, temperature among those not published. 
...
health economics working papers are not actually less likely to be published, so it's not clear what significance these words play.  The second through seventh words have a connection to public economics, with some successful papers relating to tax credits (some unanticipated).  
...
empiricism does not play an obvious role here, yet the issue could do wth further digging.


## Empiricism

Economics is far, far more of an empirical discpline than when the NBER series began in 1973:
let's show test this <i>empirically</i> beyond word counts or ratios.

Sentiment analysis is commonly applied to text data as an easy coding operation
to measure -- commonly to measure happiness, or perhaps [1am angry tweets](http://varianceexplained.org/r/trump-tweets/#fnref:fullcode).
The same process can textually measure rate of empiricism.

First I'll form a list of 100 words most likely to appear in abstracts that use the words
``empirical'', ``instrument'', ``metrics'' -- and call these empirical papers --
and top 100 in ones that don't -- non-empirical papers.  
This plays the role of a sentiment dictionary, say a pseudo lexicon, with a log ratio of inclusion 
giving a score of likelihood to be in an empirical paper -- a value of 100 is the most likely to be a word
in an empirical paper, -100 in a non-empirical paper.
```{r, echo=F}
# Which words coincide with data or empirical?
empirical_words <- Papers_abtracts.data %>%
  mutate(empirical = 0 < (
    stri_detect_fixed(abstract, 'empiric') + 
    stri_detect_fixed(abstract, 'instrument') + 
    stri_detect_fixed(abstract, 'metric'))) %>%
  select(empirical, abstract) %>%
  unnest_tokens(word, abstract, token = 'regex', pattern = reg) %>%
  filter(!word %in% c(stop_words$word, 'working', 'paper'), str_detect(word, '[a-z]')) %>%
  group_by(empirical, word) %>% summarise(count = n()) %>%
  arrange(-count) %>%
  filter(count >= 5) %>%
  ...
  arrange(logratio) %>% group_by(logratio > 0) %>% top_n(200, abs(logratio)) %>%
  ungroup() %>% mutate(word = reorder(word, logratio))
```

Here are the top 10 for empirical papers.
```{r, echo=F}
empirical_words %>% arrange(-logratio) %>% select(word, logratio) %>% head(10) %>%
  transmute(word = paste0(word, ' : ', as.character(round(logratio)))) %>% pull(word) %>% 
              paste(sep = ', ', collapse = ', ') %>% print()
```

And for non-empirical papers.
```{r, echo=F}
empirical_words %>% arrange(logratio) %>% select(word, logratio) %>% head(10) %>%
  transmute(word = paste0(word, ' : ', as.character(round(logratio)))) %>% pull(word) %>% 
              paste(sep = ', ', collapse = ', ') %>% print()
...
Take a step back and make sure the words make sense -- one would hope that ``econometrics'' is an empirical word!

Now use the log ratio measure of empiricism to clasify the papers with a sentiment measure of empiricism:
```{r, echo=T}
...
and that this can be tested.



### Conclusion: the wider economics publishing pipeline

Word stemming maybe for future deeper analysis -- too detailed for a blog post.
