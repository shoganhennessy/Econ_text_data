---
title: "Working Papers in Economics Publishing: NBER Series"
...
```{r setup, include=F}
knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning = FALSE, cache = TRUE, dev = 'svg')
library(tidyverse)
library(data.table)
library(stringi)
library(ggplot2)
...
<!-- Look at aource code : https://github.com/dgrtwo/dgrtwo.github.com/blob/master/_R/2016-08-09-trump-tweets.Rmd --> 
...
to hundreds of research papers in the 2000s.
...
And by area of expertise (note: papers can be cross referenced, and are likely to be so in more recent years).
```{r, echo=F}
...
The majoriy of papers in every year go on to be published.
The most recent years (2015 and on) have a very low publishing rate, likely to yet be published?
...
Couathorship is much more common today than three decades ago.
...
Textual analysis along different measures:

Let's compare textual measures (sentiment, relative word usage, measure of mathematical/empricial words) across dimensions:
published vs unpublished, number of authors, areas of interest etc.
